...
class _PPICListPageState extends State<PPICListPage> {
  ...
  Future<void> _fetchInitialData() async {
    try {
      final mats = await supabase
          .from('material')
          .select('material_id, material_name, box_per_pallet');
  ...
  Future<void> _fetchData({bool isSilent = false}) async {
    ...
    try {
      ...
      for (var row in rawData) {
        ...
        String key =
            "${mat['material_id']}_${row['mesin_id']}_${form['production_type']}";
        ...
        if (shift == 'I')
          grouped[key]!['qty_shift_1'] += qty;
        else if (shift == 'II')
          grouped[key]!['qty_shift_2'] += qty;
        else if (shift == 'III')
          grouped[key]!['qty_shift_3'] += qty;
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                DropdownSearch<Map<String, dynamic>>(
                                  ...
                                  itemAsString: (i) =>
                                      "${i['material_id']} - ${i['material_name']}",
                                  compareFn: (i, s) =>
                                      i['material_id'].toString() ==
                                      s['material_id'].toString(),
  ...
  Widget _buildTableCard(String title, String type) {
    final filtered = _displayData
        .where((d) => d['production_type'] == type)
        .toList();
    ...
    try {
      ...
      for (var item in _displayData) {
        sheetObject.appendRow([
          ...
          TextCellValue(
            item['production_type']?.toString().toUpperCase() ?? "-",
          ),
      ...
      String fileName =
          "Rekap_PPIC_${DateFormat('yyyyMMdd').format(_selectedDate)}.xlsx";
  ...
  Future<void> _import() async {
    try {
      ...
      for (var i = 1; i < sheet.maxRows; i++) {
        ...
        String type =
            row[colMap["type"]!]?.value.toString().toLowerCase() ?? "";
        String shift = row[colMap["shift"]!]?.value.toString() ?? "";
        String mesinRaw = row[colMap["mesin"]!]?.value.toString() ?? "";
        int matId =
            int.tryParse(row[colMap["material"]!]?.value.toString() ?? "0") ??
            0;
        int qty =
            int.tryParse(row[colMap["qty"]!]?.value.toString() ?? "0") ?? 0;
        ...
        if (finalMesinId == null && mesinRaw.isNotEmpty) {
          try {
            finalMesinId = _mesinList.firstWhere(
              (m) =>
                  m['nama_mesin'].toString().toLowerCase() ==
                  mesinRaw.toLowerCase(),
            )['mesin_id'];
          } catch (_) {
            finalMesinId = 0;
          }
