...
class _ListDOPageState extends State<ListDOPage> {
  ...
  void _editShippingRequest(Map<String, dynamic> req) async {
    ...
    if (isGroup) {
      for (int sId in idsToUpdate) {
        var originalReq = _allRequests.firstWhere(
          (element) => element['shipping_id'] == sId,
        );
        soController[sId] = TextEditingController(
          text: originalReq['so']?.toString() ?? "",
        );
      }
    } else {
      soController[req['shipping_id']] = TextEditingController(
        text: req['so']?.toString() ?? "",
      );
    }
    ...
    for (var d in dos) {
      for (var det in d['do_details']) {
        ...
        qtyControllers[detId] = TextEditingController(
          text: det['qty'].toString(),
        );
      }
    }

    DateTime? selectedRDD = req['rdd'] != null
        ? DateTime.tryParse(req['rdd'].toString())
        : null;
    DateTime? selectedStuffing = req['stuffing_date'] != null
                      ...
                      Row(
                        children: [
                          _buildDateTile(
                            "RDD",
                            selectedRDD,
                            (date) => setModalState(() => selectedRDD = date),
                          ),
                          ...
                          _buildDateTile(
                            ...
                            selectedStuffing,
                            (date) =>
                                setModalState(() => selectedStuffing = date),
                          ),
                        ],
                      ),
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    try {
                      for (var entry in soController.entries) {
                        await supabase
                            ...
                            .update({
                              ...
                              'stuffing_date': selectedStuffing
                                  ?.toIso8601String(),
                            })
                      ...
                      _showSnackBar(
                        "Data dan Qty Berhasil Diperbarui!",
                        Colors.green,
                      );
                      _fetchShippingRequests();
                    } catch (e) {
  ...
  Future<void> _createAndAssignGroup() async {
    ...
    final alreadyGrouped = _allRequests
        .where(
          (req) =>
              _selectedIds.contains(req['shipping_id']) &&
              req['group_id'] != null,
        )
        .toList();

    if (alreadyGrouped.isNotEmpty) {
      String problemIds = alreadyGrouped
          .map((e) => e['shipping_id'])
          .join(", ");
    ...
    for (var req in source) {
      if (req['group_id'] == null) {
        ...
        if (!groupedMap.containsKey(gId)) {
          ...
          List historyWithId = historyRaw
              .map((h) => {...h, 'origin_ship_id': req['shipping_id']})
              .toList();
          ...
          List currentHistory = List.from(
            groupedMap[gId]!['collective_history'] ?? [],
          );
  ...
  Future<void> _editStuffingMassal() async {
    ...
    if (picked != null) {
      ...
      try {
        ...
        _showSnackBar(
          "Berhasil update stuffing ${_selectedIds.length} data",
          Colors.green,
        );
  ...
  Future<void> _prosesKePermintaan() async {
    ...
    try {
      ...
      for (var id in _selectedIds) {
        ...
        if (req.isNotEmpty) {
          ...
          if (so.isEmpty || rdd.isEmpty || stuffing.isEmpty) {
            errorMessages.add(
              "Ship ID $id: SO, RDD, atau Stuffing Date masih kosong.",
            );
            continue;
          }
        }
      ...
      _showSnackBar(
        "Berhasil memproses ${_selectedIds.length} Shipping ID",
        Colors.green,
      );
    ...
    setState(() {
      ...
      _filteredRequests = _allRequests.where((req) {
        ...
        bool matchInDO = dos.any((doItem) {
          ...
          final custId = (doItem['customer']?['customer_id'] ?? "")
              .toString()
              .toLowerCase();
          final custName = (doItem['customer']?['customer_name'] ?? "")
              .toString()
              .toLowerCase();
          final List details = doItem['do_details'] ?? [];

          bool matchMat = details.any((det) {
            final matId = (det['material']?['material_id'] ?? "")
                .toString()
                .toLowerCase();
            final matName = (det['material']?['material_name'] ?? "")
                .toString()
                .toLowerCase();

            return matId.contains(_searchQuery) ||
                matName.contains(_searchQuery);
          });
        ...
        if (_selectedDateRange != null) {
          String dateColumn = _dateFilterType == "RDD"
              ? 'rdd'
              : 'stuffing_date';
                ...
                Container(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      isDense: true,
                      dropdownColor: _selectedDateRange != null
                          ? Colors.red.shade800
                          : Colors.white,
                      iconEnabledColor: _selectedDateRange != null
                          ? Colors.white
                          : Colors.black87,
                      style: TextStyle(
                        fontSize: 12,
                        ...
                        child: ClipRect(
                          child: DataTable(
                            ...
                            columns: originalColumns.map((col) {
                              ...
                              return DataColumn(
                                label: SizedBox(width: targetWidth),
                              );
                            }).toList(),
                            rows: _getGroupedDisplayData(_filteredRequests).map(
                              (req) {
  ...
  DataRow _buildDataRow(Map<String, dynamic> req) {
    ...
    for (var d in dos) {
      String currentSo =
          d['parent_so']?.toString() ?? req['so']?.toString() ?? "-";
      ...
      for (var det in d['do_details']) {
        ...
        doNumW.add(
          _buildTextItem(d['do_number'] ?? "-", isBold: true, width: 80),
        );
        custIdW.add(_buildTextItem(custId, width: 70));
        custW.add(
          _buildTextItem(d['customer']?['customer_name'] ?? "-", width: 180),
        );
        matIdW.add(
          _buildTextItem(
            det['material']?['material_id']?.toString() ?? "-",
            width: 65,
          ),
        );
        matW.add(
          _buildTextItem(det['material']?['material_name'] ?? "-", width: 220),
        );
        matTypeW.add(
          _buildTextItem(det['material']?['material_type'] ?? "-", width: 45),
        );
        qtyW.add(
          _buildTextItem(
            det['qty']?.toString() ?? "0",
    ...
    return DataRow(
      ...
      color: WidgetStateProperty.resolveWith<Color?>((states) {
        ...
        if (req['group_id'] != null)
          return Colors.blue.shade100.withValues(alpha: 0.5);
        return null;
      }),
      cells: [
        DataCell(
          ...
          Column(
            ...
            children: [
              ...
              _buildStatusBadge(
                req['status'],
                isGroupRow
                    ? req['collective_history']
                    : req['shipping_pending_history'],
              ),
              if (isGroupRow)
                Container(
        ...
        DataCell(
          Text(
            formatSmart(totalNetWeight / 1000),
  ...
  List<DataColumn> _buildColumns() {
    ...
    return [
      ...
      const DataColumn(
        label: SizedBox(width: 180, child: Text('Customer Tujuan')),
      ),
      const DataColumn(label: SizedBox(width: 65, child: Text('No Mat'))),
      const DataColumn(
        label: SizedBox(width: 220, child: Text('Nama Material')),
      ),
  ...
  Future<void> _selectDate(
    ...
    DateTime initialDate =
        DateTime.tryParse(req[fieldName]?.toString() ?? "") ?? DateTime.now();
    ...
    if (confirm) {
      try {
        ...
        if (isGroup) {
          ...
          await supabase.from('shipping_groups').delete().eq('id', groupId);
        } else {
          await supabase
              .from('shipping_request')
              .delete()
  ...
  void _showReasonDialog(List history) {
    ...
    sortedHistory.sort((a, b) {
      DateTime dtA =
          DateTime.tryParse(a['pending_at']?.toString() ?? "") ??
          DateTime(2000);
      DateTime dtB =
          DateTime.tryParse(b['pending_at']?.toString() ?? "") ??
          DateTime(2000);
        ...
        content: SizedBox(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              DateTime date =
                  DateTime.tryParse(item['pending_at']?.toString() ?? "") ??
                  DateTime.now();
              String originId =
                  (item['origin_ship_id'] ??
                          item['shipping_id'] ??
  ...
  Widget _buildStatusBadge(String? status, dynamic historyData) {
    ...
    history.sort(
      (a, b) => (a['pending_at'] ?? "").compareTo(b['pending_at'] ?? ""),
    );
  ...
  Future<void> _splitGroup() async {
    if (_selectedIds.isEmpty) {
      _showSnackBar(
        "Pilih data yang ingin dipisahkan dari grup",
        Colors.orange,
      );
      return;
    }
    ...
    try {
      ...
      if (selectedGroups.isEmpty) {
        setState(() => _isLoading = false);
        _showSnackBar(
          "Data yang dipilih memang tidak masuk dalam grup mana pun",
          Colors.blueGrey,
        );
        return;
      }
  ...
  Future<void> _importMassalTigaTabel() async {
    try {
      ...
      for (var entry in groupedByDO.entries) {
        ...
        List<Map<String, dynamic>> detailsToInsert = items
            .map(
              (item) => {
                'do_id': newDoId,
      ...
      if (duplicateDOs.isNotEmpty || materialNotFoundDOs.isNotEmpty) {
        ...
        if (duplicateDOs.isNotEmpty) {
          feedbackMessage +=
              "⚠️ DO Duplikat (Dilewati): ${duplicateDOs.join(', ')}\n";
        }
        if (materialNotFoundDOs.isNotEmpty) {
          feedbackMessage +=
              "❌ Material Tidak Ditemukan (Dilewati): ${materialNotFoundDOs.join(', ')}";
        }

        _showSnackBar(feedbackMessage.trim(), Colors.orange.shade900);
      } else {
        _showSnackBar(
          "Berhasil import massal semua data tanpa kendala!",
          Colors.green,
        );
      }
    } catch (e) {
