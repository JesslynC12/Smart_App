...
class _LogisticDashboardPageState extends State<LogisticDashboardPage> {
  ...
  Future<void> _fetchDashboardData({bool isRealtimeTrigger = false}) async {
    ...
    checker (
      ...
      shift
          ...
          ''')
          .not(
            'shipping_assignments.status_assignment',
            'in',
            '(rejected,"rejected unit","no response","cancel booking")',
          )
          .eq(
            'shipping_assignments.loading.verifikasi_rekomendasi_logistic',
            'OKE',
          )
  ...
  List<DataColumn> _buildColumns() {
    return const [
      ...
      DataColumn(
        label: SizedBox(width: 125, child: Text('Vendor Transportasi')),
      ),
      ...
      DataColumn(
        label: SizedBox(width: 130, child: Text('No Segel Pelayaran')),
      ),
  ...
  DataRow _buildDataRow(Map<String, dynamic> req) {
    ...
    Map<String, dynamic>? assignment = assignmentsList.isNotEmpty
        ? assignmentsList.first
        : null;

    String warehouseName = req['warehouse']?['warehouse_name'] ?? "-";
    String vendorName =
        assignment?['vendor_transportasi']?['vendor_name'] ?? "-";
    String typeUnit = assignment?['vendor_transportasi']?['type_unit'] ?? "-";
    String winnerRank =
        (assignment?['vendor_transportasi']?['winner_rank'] ?? "-").toString();
    ...
    Map<String, dynamic>? loadingInfo = loadingList.isNotEmpty
        ? loadingList.first
        : null;
    ...
    for (var d in dos) {
      ...
      for (int i = 0; i < details.length; i++) {
        ...
        double nwValue =
            ...
            0;
        //double calculatedTnw = (qty * nwValue) / 1000;
        ...
        doNumW.add(
          _buildCellText(d['do_number'] ?? "-", isBold: true, width: 80),
        );
        if (i == 0) {
        ...
        matW.add(
          _buildCellText(det['material']?['material_name'] ?? "-", width: 210),
        );
        matTypeW.add(
          _buildCellText(det['material']?['material_type'] ?? "-", width: 80),
        );
        divDescW.add(
          _buildCellText(
            det['material']?['division_description'] ?? "-",
            width: 130,
          ),
        );
    ...
    return DataRow(
      color: WidgetStateProperty.resolveWith(
        (states) =>
            isGroupRow ? Colors.blue.shade50.withValues(alpha: 0.3) : null,
      ),
      cells: [
        DataCell(
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...idsInRow.map(
                  (id) => Text(id.toString(), style: TextStyle(fontSize: 11)),
                ),
                if (isGroupRow)
                  Container(
        ...
        DataCell(
          Text(
            _formatDateOnly(req['stuffing_date']),
        ...
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: doNumW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: soW),
        ),
        DataCell(
          Column(
            mainAxisAlignment: MainAxisAlignment.center,
        ...
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: custW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: cityW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: areaW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: matIdW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: matW),
        ),
        DataCell(
          Column(
            mainAxisAlignment: MainAxisAlignment.center,
        ...
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: qtyW),
        ),
        DataCell(
          Column(mainAxisAlignment: MainAxisAlignment.center, children: nwW),
        ),
        DataCell(
          Center(
            child: Text(
    ...
    try {
      ...
      for (var req in _filteredRequests) {
        sheet.appendRow([
          ...
          TextCellValue(
            req['delivery_order']?[0]?['customer']?['customer_name'] ?? "-",
          ),
      ...
      final fileName =
          "Logistik_Export_${DateFormat('yyyyMMdd').format(DateTime.now())}.xlsx";
