            ...
            *,
            ...
            shipping_assignments (
              ...
              loading (
                id_loading,
                checker_id,
                checker (checker_id, checker_name, shift)
              )
            )
          ''')
...
//const double totalTableWidth = 2400.0;
  ...
  List<DataColumn> _buildColumns() {
    return const [
      ...
      DataColumn(label: SizedBox(width: 60, child: Text('TNW'))),
      ...
      DataColumn(label: SizedBox(width: 110, child: Text('Check In'))),
      DataColumn(label: SizedBox(width: 110, child: Text('Keluar'))),
  ...
  DataRow _buildDataRow(Map<String, dynamic> req) {
    ...
    List<Widget> doNumW = [], soW = [], custIdW = [], custW = [], matIdW = [], matW = [], matTypeW = [], qtyW = [], nwW = [], tnwW = [];
    ...
    for (var d in dos) {
      ...
      for (var det in d['do_details']) {
        ...
        custW.add(_buildCellText(d['customer']?['customer_name'] ?? "-", width: 180));
        ...
        matTypeW.add(_buildCellText(det['material']?['material_type'] ?? "-", width: 50));
...
// --- EXCEL EXPORT LOGIC ---
  ...
  String _formatDateTime(dynamic dateTimeStr) {
    ...
    try {
      return DateFormat('dd/MM HH:mm').format(DateTime.parse(dateTimeStr.toString()).toLocal());
    } catch (_) {
      return "-";
    }
  }
