...
class _VendorRequestPageState extends State<VendorRequestPage> {
  ...
  void initState() {
    ...
    _fetchWarehouse();
    _initNotification();
    _fetchVendorTargetData(showGlobalLoading: true);
    _setupRealtimeChannel();
  }

  void _setupRealtimeChannel() {
    _realtimeChannel = supabase
        .channel('vendor_request_unified_realtime')
        .onPostgresChanges(
          event: PostgresChangeEvent.all,
          schema: 'public',
          table: 'shipping_request',
          callback: (payload) async {
            await _fetchVendorTargetData();
          },
        )
        .onPostgresChanges(
          event: PostgresChangeEvent.all,
          ...
          callback: (payload) async {
            if (payload.newRecord.isNotEmpty) {
              final statusAss = payload.newRecord['status_assignment'];
              if (statusAss == 'rejected') {
                _showRejectNotification(
                  "Transportasi",
                  payload.newRecord['shipping_id'] ?? 0,
                );
              }
            }
            await _fetchVendorTargetData();
  ...
  Future<void> _fetchVendorTargetData({bool showGlobalLoading = false}) async {
    try {
      ...
      if (mounted) {
        setState(() {
          List<Map<String, dynamic>> rawGrouped = _getGroupedDisplayData(
            ...
            for (var originalRow in (response as List)) {
              if (groupShipIds.contains(originalRow['shipping_id'])) {
                final List assignments =
                    originalRow['shipping_assignments'] as List? ?? [];

                for (var a in assignments) {
                  final String statusAss =
                      a['status_assignment']?.toString().toLowerCase() ?? "";
          ...
          rawGrouped.sort((a, b) {
            bool aHasReject =
                (a['unique_reject_list'] as List? ?? []).isNotEmpty;
            bool bHasReject =
                (b['unique_reject_list'] as List? ?? []).isNotEmpty;
            ...
            child: _isLoading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildVendorCard(_dataList[index]),
                  ),
          ),
        ],
      ),
                  ...
                  Container(
                    ...
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        isDense: true,
                        dropdownColor: isDateActive
                            ? Colors.red.shade800
                            : Colors.white,
                        iconEnabledColor: isDateActive
                            ? Colors.white
                            : Colors.black87,
                        style: TextStyle(
                          fontSize: 10,
              ...
              children: [
                Row(
                  children: [
                    _infoText(
                      "🚛 Stuffing:",
                      _formatDate(item['stuffing_date']),
                    ),
                ...
                ...dos.map((doItem) {
                  ...
                  final String rddSpesifik = _formatDate(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ...rejectHistory.map((rej) {
                          String vendorName =
                              rej['vendor_transportasi']?['vendor_name'] ??
                              "Unknown Vendor";

                          String typeText = "";
                          String status =
                              rej['status_assignment']
                                  ?.toString()
                                  .toLowerCase() ??
                              "";
                          ...
                          if (status == 'rejected unit') {
                            ...
                            var alasanCancel =
                                rej['cancelled_reason'] ?? rej['catatan'];
          ...
          Padding(
            ...
            child: ElevatedButton.icon(
              ...
              onPressed: () {
                ...
                DynamicTabPage.of(
                  context,
                )?.openTab(tabTitle, AssignVendorPage(shippingId: shipId));
              },
  ...
  InputDecoration _filterInputDecoration(String label) => InputDecoration(
    labelText: label,
