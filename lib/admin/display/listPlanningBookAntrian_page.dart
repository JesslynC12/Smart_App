...
class BookingPlanningListPage extends StatefulWidget {
  ...
  @override
  State<BookingPlanningListPage> createState() =>
      _BookingPlanningListPageState();
}
...
class _BookingPlanningListPageState extends State<BookingPlanningListPage> {
    ...
    checker_name
      ...
      for (var item in response) {
        ...
        String currentVendorName =
            item['vendor_transportasi']?['vendor_name'] ?? 'Unknown Vendor';
        ...
        if (!groupedData.containsKey(key)) {
          ...
          for (var newDo in currentItemDOs) {
            bool isAlreadyExists = existingDOs.any(
              (ext) => ext['do_number'] == newDo['do_number'],
            );
            if (!isAlreadyExists) {
          ...
          int existingId =
              int.tryParse(groupedData[key]['id_assignment'].toString()) ?? 0;
          ...
          List<Map<String, dynamic>> existingLogs =
              List<Map<String, dynamic>>.from(
                groupedData[key]['history_logs'] ?? [],
              );
          bool isAlreadyLogged = existingLogs.any(
            (log) =>
  ...
  Widget _buildPlanningCard(Map<String, dynamic> item) {
    ...
    final loadingData = item['loading'] is List
        ? (item['loading'] as List).isNotEmpty
              ? item['loading'][0]
              : {}
        : item['loading'] ?? {};
            ...
            child: Column(
              ...
              children: [
                          ...
                          if (item['vendor_transportasi'] != null)
                            Padding(
                              ...
                              child: Wrap(
                                ...
                                children: [
                                  _miniVendorDetail(
                                    "QCF: ${item['vendor_transportasi']['qcf'] ?? '-'}",
                                  ),
                                  _miniVendorDetail(
                ...
                if ([
                    ...
                    (item['history_logs'] != null &&
                        (item['history_logs'] as List).isNotEmpty))
                  Container(
                    width: double.infinity,
                    padding: const EdgeInsets.all(12),
                              ...
                              if (item['history_logs'] != null)
                                ...(item['history_logs'] as List).map((log) {
                                  ...
                                  if (hStatus == 'no response') {
                                    msg =
                                        "Vendor ($hVendor) tidak merespon penugasan hingga batas waktu.";
                                  } else if (hStatus == 'rejected') {
                                    msg =
                                        "Penugasan ditolak oleh Vendor $hVendor karena $hReason.";
                                    ...
                                    msg =
                                        "Booking dibatalkan oleh $by karena $r.";
                                  } else if (hStatus == 'rejected unit') {
                                    msg =
                                        "Unit Feasibility Check ditolak untuk Vendor $hVendor.";
                                  }
                        ...
                        if (status == 'loading' ||
                          ...
                          Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  "Checker: ${loadingData['checker']?['checker_name'] ?? '-'}",
                                  style: TextStyle(
                                    fontSize: 11,
                                    color: Colors.orange.shade900,
                                    fontWeight: FontWeight.w600,
        ...
        builder: (context, setStateDialog) => AlertDialog(
              ...
              children: [
                ..._rescheduleReasons.map(
                  (reason) => RadioListTile<String>(
                    ...
                    onChanged: (val) =>
                        setStateDialog(() => _tempCancelReason = val),
                  ),
                ),
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: _tempCancelReason == null
                  ? null
                  : () async {
  ...
  Future<void> _executeCancelBooking(
    ...
    try {
      ...
      final String activeUserName =
          profileRes?['name'] ?? currentUser?.email ?? 'Admin';
      final List<int> shippingIds = List<int>.from(
        item['grouped_shipping_ids'] ?? [item['request']['shipping_id']],
      );
      ...
      if (assignmentIds.isEmpty) {
        _showSnackBar(
          "Tidak ada penugasan aktif yang bisa dibatalkan",
          Colors.orange,
        );
        return;
      }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text(
            "Tidak ada antrian planning saat ini",
