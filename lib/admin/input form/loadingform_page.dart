...
class _LoadingFormPageState extends State<LoadingFormPage> {
          ...
          Row(
            children: [
              ...
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                decoration: BoxDecoration(
                  color: item['decision_for_unit'] == 'LAYAK'
                      ? Colors.green
                      : Colors.orange,
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              list.isEmpty ? "OK / Standar" : list.join(", "),
              style: TextStyle(
                fontSize: 11,
  ...
  Future<void> _submitLoadingData() async {
    if (_rekomendasiLogistic == null) {
      _showSnackBar(
        "Harap pilih Verifikasi Rekomendasi Logistic!",
        Colors.orange,
      );
      return;
    }
    ...
    if (_rekomendasiLogistic == 'OKE') {
      ...
      if (!isFormValid || !isGanjalValid) {
        _showSnackBar(
          "Mohon lengkapi Checker, No Segel, dan Ganjal Ban!",
          Colors.orange,
        );
        return;
      }
    ...
    try {
      final List<int> assignmentIds = List<int>.from(
        widget.item['grouped_assignment_ids'] ?? [widget.item['id_assignment']],
      );
      final List<int> shipIds = List<int>.from(
        widget.item['grouped_shipping_ids'] ?? [widget.item['shipping_id']],
      );
      final List<Map<String, dynamic>> loadingEntries = assignmentIds.map((id) {
        return {
          ...
          'checker_id': _rekomendasiLogistic == 'OKE'
              ? int.parse(_selectedChecker!)
              : null,
          'ganjal_ban': _rekomendasiLogistic == 'OKE' ? _ganjalBan : null,
          'no_segel_smart':
              (_rekomendasiLogistic == 'OKE' &&
                  _noSegelController.text.isNotEmpty)
      ...
      if (_rekomendasiLogistic == 'OKE') {
        await supabase
            .from('shipping_assignments')
            .update({'status_assignment': 'loading'})
      ...
      if (mounted) {
        ...
        _showSnackBar(
          msg,
          _rekomendasiLogistic == 'OKE' ? Colors.green : Colors.blue,
        );
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: _rekomendasiLogistic == 'OKE'
          ...
          child: _isSubmitting
              ...
              : Text(
                  isOke
                      ? "KONFIRMASI SELESAI (OKE)"
                      : "CATAT RIWAYAT (BELUM OKE)",
                  style: const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                  ),
