...
class _ComplainPageState extends State<ComplainPage> {
  ...
  Future<void> _searchDO() async {
    ...
    ''')
      ...
      final assignment = (assignmentData as List).isNotEmpty
          ? (assignmentData as List)[0]
          : null;

      String checkerName = "-";
      if (assignment?['loading'] != null &&
          (assignment!['loading'] as List).isNotEmpty) {
      ...
      setState(() {
        ...
        _headerData = {
          ...
          "stuffing":
              (allDos as List)[0]['shipping_request']?['stuffing_date'] ?? "-",
          "rdd": (allDos as List)[0]['shipping_request']?['rdd'] ?? "-",
          "gudang":
              (allDos
                  as List)[0]['shipping_request']?['warehouse']?['warehouse_name'] ??
              "-",
          "checker": checkerName,
          "jam_in": assignment?['checkIn_at'] != null
              ? DateFormat(
                  'HH:mm',
                ).format(DateTime.parse(assignment['checkIn_at']))
              : "-",
          "jam_out": assignment?['keluar_at'] != null
              ? DateFormat(
                  'HH:mm',
                ).format(DateTime.parse(assignment['keluar_at']))
              : "-",
        };
      });
    } catch (e) {
              ...
              child: Column(
                children: [
                  ...
                  if (_isDataLoaded) ...[
                    ...
                    const SizedBox(height: 12),
                    const Text(
                      "Pilih material di bawah untuk dikomplain:",
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.red,
                    ...
                    const Padding(
                      ...
                      child: Text(
                        "Cari No DO untuk menampilkan rekap data",
          ...
          children: [
            ...
            Table(
              ...
              children: [
                _buildTableRow(
                  "Driver / HP",
                  "${_headerData['supir']} (${_headerData['hp_supir']})",
                ),
                _buildTableRow(
                  "No. Polisi / Thn",
                  "${_headerData['no_polisi']} / ${_headerData['tahun']}",
                ),
                _buildTableRow("Vendor / Transporter", _headerData['vendor']),
                _buildTableRow(
                  "Stuffing / RDD",
                  "${_headerData['stuffing']} / ${_headerData['rdd']}",
                ),
                _buildTableRow("Customer Tujuan", _headerData['customer']),
                _buildTableRow(
                  "Jam In / Out",
                  "${_headerData['jam_in']} - ${_headerData['jam_out']}",
                ),
        ...
        children: [
          ...
          ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              bool isSelected =
                  _selectedMaterial?['details_id'] == item['details_id'];
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: ["Penolakan", "Kekurangan", "Kelebihan"]
                  .map(
                    (v) => DropdownMenuItem(
                      value: v,
