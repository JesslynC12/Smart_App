...
class _WarehousePaginatedPageState extends State<WarehousePaginatedPage> {
  ...
  Future<void> _exportWarehouseToExcel() async {
    ...
    try {
      ...
      String fileName =
          "Master_Warehouse_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _importWarehouseFromExcel() async {
    try {
      ...
      if (importData.isNotEmpty) {
        ...
        _showMsg(
          "Berhasil import ${importData.length} data gudang",
          Colors.green,
        );
        _fetchData();
      } else {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (number != null) {
        query = query.or(
          'wh_code.eq.$number, warehouse_name.ilike.%$_searchQuery%',
        );
      } else {
  ...
  Future<void> _processSave(
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isEdit ? "Data diperbarui" : "Data disimpan"),
  ...
  void _showFormDialog([Map<String, dynamic>? warehouse]) {
    ...
    final codeController = TextEditingController(
      text: warehouse?['wh_code']?.toString() ?? '',
    );
    final nameController = TextEditingController(
      text: warehouse?['warehouse_name'] ?? '',
    );
    final kapasitasController = TextEditingController(
      text: warehouse?['kapasitas']?.toString() ?? '',
    );
    final maxUtilizeController = TextEditingController(
      text: warehouse?['max_utilize']?.toString() ?? '',
    );
    final tipeController = TextEditingController(
      text: warehouse?['tipe'] ?? '',
    );

    String rawStatus = (warehouse?['status'] ?? 'active')
        .toString()
        .toLowerCase();
    String selectedStatus = rawStatus == 'inactive' ? 'inactive' : 'active';

    String rawLokasi = (warehouse?['lokasi'] ?? 'Rungkut').toString();
    String selectedLokasi = ['Rungkut', 'Tambak Langon'].contains(rawLokasi)
        ? rawLokasi
        : 'Rungkut';
  ...
  void _validateAndSave(
    ...
    if (code.text.isEmpty || name.text.isEmpty) {
      _showMsg(
        "WH Code, Nama Warehouse, dan Lokasi wajib diisi!",
        Colors.orange,
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    final source = WarehouseDataSource(
      _warehouses,
      context,
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () => _confirm(wh['warehouse_id']),
              ),
            ],
          ),
        ),
