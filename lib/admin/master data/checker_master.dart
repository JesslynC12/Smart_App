...
class _CheckerPaginatedPageState extends State<CheckerPaginatedPage> {
  ...
  Future<void> _exportCheckerToExcel() async {
    ...
    try {
      ...
      String fileName =
          "Master_Checker_${DateTime.now().millisecondsSinceEpoch}.xlsx";
  ...
  Future<void> _importCheckerFromExcel() async {
    try {
      ...
      if (importData.isNotEmpty) {
        ...
        _showMsg(
          "Berhasil import ${importData.length} data checker",
          Colors.green,
        );
        _fetchData();
      } else {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        if (isNumber) {
          query = query.or(
            'checker_id.eq.$_searchQuery, checker_name.ilike.%$_searchQuery%',
          );
        } else {
  ...
  void _showFormDialog([Map<String, dynamic>? checker]) {
    ...
    final nameController = TextEditingController(
      text: checker?['checker_name'] ?? '',
    );
    final shiftController = TextEditingController(
      text: (checker?['shift'] ?? '').toString(),
    );
    ...
    String rawStatus = (checker?['status'] ?? 'active')
        .toString()
        .toLowerCase();
    String selectedStatus = ['active', 'inactive'].contains(rawStatus)
        ? rawStatus
        : 'active';
  ...
  Widget build(BuildContext context) {
    final source = CheckerDataSource(
      _checkers,
      context,
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () => _confirm(item['checker_id']),
              ),
            ],
          ),
        ),
